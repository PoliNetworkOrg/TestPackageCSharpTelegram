...
public static class DateTimeUtil
{
    ...
    public static string NowAsStringAmericanFormat()
    {
        var dt = DateTime.Now;
    ...
    public static DateTime? GetHours(string? s)
    {
        var dt = GetDateTimeFromString(s);
        return dt?.Item1;
    }

    public static string? DateTimeToAmericanFormat(DateTime? dt)
    {
        if (dt == null)
            return null;
    ...
    public static DateTime? GetFromString(string? v)
    {
        if (v != null && v.Contains(' '))
        {
            ...
            var data1 = v2[0].Split('-');
            if (!v2[1].Contains('.')) return null;
            var v4 = v2[1].Split('.');
            if (!v4[0].Contains(':')) return null;
            var orario = v4[0].Split(':');
            var d2 = new DateTime((int)Convert.ToInt64(data1[0]), (int)Convert.ToInt64(data1[1]),
                (int)Convert.ToInt64(data1[2]), (int)Convert.ToInt64(orario[0]),
        ...
        var data2 = v.Split('-');
        var d1 = new DateTime((int)Convert.ToInt64(data2[0]), (int)Convert.ToInt64(data2[1]),
            (int)Convert.ToInt64(data2[2]));
        return d1;
    }


    public static DateTime? GetDateTime(IReadOnlyList<string?>? target)
    {
        if (target == null)
            return null;
        if (target.Count < 3)
